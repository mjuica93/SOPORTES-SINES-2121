...
# -*- coding: utf-8 -*-
"""
Servidor Seguro Simplificado para Sistema SINES
...
class SimpleSecureHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_validate_admin(self):
        """Validar permisos de administrador usando cookies"""
        try:
            # Obtener sesión desde cookies
            session = self.validate_session_from_cookies()
    ...
    def set_session_cookie(self, session_id):
        """Establecer cookie de sesión"""
        self.send_header('Set-Cookie', f'sines_session={session_id}; Path=/; HttpOnly; SameSite=Lax')

    def clear_session_cookie(self):
        """Limpiar cookie de sesión"""
        self.send_header('Set-Cookie', 'sines_session=; Path=/; HttpOnly; SameSite=Lax; Expires=Thu, 01 Jan 1970 00:00:00 GMT')

    def do_GET(self):
        """Manejar GET requests con seguridad mejorada"""
        
        # Rutas completamente públicas (solo login y recursos estáticos básicos)
        public_files = [
            '/index_secure_simple.html',
            '/favicon.ico'
        ]
        
        # Archivos estáticos básicos (CSS, JS, imágenes) - sin datos sensibles
        ...
        # API de estado público (información básica)
        ...
        if any(self.path.startswith(path) for path in admin_paths):
            if session['username'] not in ['admin', 'supervisor']:
                ...
                <body style="font-family: Arial; text-align: center; padding: 50px;">
                    ...
                    <button onclick="window.location.href='/index_secure_simple.html'">Volver al Dashboard</button>
                </body>
                </html>
                """.encode('utf-8'))
                return
...
def start_secure_server():
    PORT = 8002  # Cambiar a puerto 8002 para evitar conflictos
    
    try:
        with socketserver.TCPServer(("", PORT), SimpleSecureHandler) as httpd:
            print("🔒 SERVIDOR SEGURO SINES - VERSIÓN SIMPLE")
            print("=" * 50)
            print(f"🌐 URL: http://localhost:{PORT}")
            print(f"🔐 Página de acceso: http://localhost:{PORT}/index_secure_simple.html")
            ...
            def open_browser():
                time.sleep(2)
                webbrowser.open(f'http://localhost:{PORT}/index_secure_simple.html')
